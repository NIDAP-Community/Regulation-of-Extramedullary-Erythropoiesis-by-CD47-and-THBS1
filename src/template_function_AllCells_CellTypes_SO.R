...
AllCells_CellTypes_SO <- function(QC_CombNorm_SO,CellDex_snapshotDate_2021_10_19, QC_CombNorm_MetadataTable) {
...
## --------- ##
    ...
    library(SCWorkflow) #added during R3 extraction
    library(grid)
    library(gridExtra)
    library(gridBase)
    library(cowplot)
...
# Primary Inputs:
CombinedSO <-QC_CombNorm_SO
Metadata <- QC_CombNorm_MetadataTable
...
print("template_function_AllCells_CellTypes_SO.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_QC_CombNorm_SO<-readRDS(paste0(rds_output,"/var_QC_CombNorm_SO.rds"))
Input_is_Seurat_count <- 1
##for(item in var_QC_CombNorm_SO){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_QC_CombNorm_SO<-as.data.frame(var_QC_CombNorm_SO)}else{var_QC_CombNorm_SO <- var_QC_CombNorm_SO}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
...
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_QC_CombNorm_MetadataTable<-NULL
Input_is_Seurat_count <- 0
##for(item in var_QC_CombNorm_MetadataTable){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_QC_CombNorm_MetadataTable<-as.data.frame(var_QC_CombNorm_MetadataTable)}else{var_QC_CombNorm_MetadataTable <- var_QC_CombNorm_MetadataTable}
invisible(graphics.off())
var_AllCells_CellTypes_SO<-AllCells_CellTypes_SO(var_QC_CombNorm_SO,var_CellDex_snapshotDate_2021_10_19,var_QC_CombNorm_MetadataTable)
invisible(graphics.off())
saveRDS(var_AllCells_CellTypes_SO, paste0(rds_output,"/var_AllCells_CellTypes_SO.rds"))
