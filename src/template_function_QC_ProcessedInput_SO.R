# Process Input Data [scRNA-seq][CCBR] (4b79d66e-50f0-42da-8c47-dd565dab2257): v47
QC_ProcessedInput_SO <- function(ccbr1072_h5_files_new,ccbr1072_MetadataToAdd) {
...
#  For standalone run:
#  libs=c(c("Seurat","stringr","dplyr","tidyr","RColorBrewer","ggplot2","ggpubr"))
#  Troubleshooting:
...
input=ccbr1072_h5_files_new
sample.metadata.table=ccbr1072_MetadataToAdd
...
print("template_function_QC_ProcessedInput_SO.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_ccbr1072_h5_files_new<-readRDS(paste0(rds_output,"/var_ccbr1072_h5_files_new.rds"))
Input_is_Seurat_count <- 1
#for(item in var_ccbr1072_h5_files_new){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
#if(Input_is_Seurat_count == 0 ){
#var_ccbr1072_h5_files_new<-as.data.frame(var_ccbr1072_h5_files_new)}else{var_ccbr1072_h5_files_new <- var_ccbr1072_h5_files_new}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_ccbr1072_MetadataToAdd<-readRDS(paste0(rds_output,"/var_ccbr1072_MetadataToAdd.rds"))
#Input_is_Seurat_count <- 0
##for(item in var_ccbr1072_MetadataToAdd){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
#if(Input_is_Seurat_count == 0 ){
#var_ccbr1072_MetadataToAdd<-as.data.frame(var_ccbr1072_MetadataToAdd)}else{var_ccbr1072_MetadataToAdd <- var_ccbr1072_MetadataToAdd}
invisible(graphics.off())
var_QC_ProcessedInput_SO<-QC_ProcessedInput_SO(var_ccbr1072_h5_files_new,var_ccbr1072_MetadataToAdd)
invisible(graphics.off())
saveRDS(var_QC_ProcessedInput_SO, paste0(rds_output,"/var_QC_ProcessedInput_SO.rds"))
