...
R04C14DEG_CD47KOvsWT_GSEAViz <- function(R04C14_DEG_Volcano_CD47KOvsWT_copied, R04C14DEG_CD47KOvsWT_GSEA, R04C14_Filtered_DEG, R04C14_Filtered_MetadataTable, GSEA_MSigDB_v6_2_with_Orthologs_CCBR_) {
...
## PARAMETERS
    ...
    genescore_df <- R04C14_DEG_Volcano_CD47KOvsWT_copied
    gex_df <- R04C14_Filtered_DEG
    metadata_df <- R04C14_Filtered_MetadataTable
    gsea_df <- R04C14DEG_CD47KOvsWT_GSEA
...
png(filename="R04C14DEG_CD47KOvsWT_GSEAViz.png", width=(pageWidth+5)*300, height=(pageHeight+5)*300, units="px", pointsize=4, bg="white", res=300, type="cairo") 
print(preview)
...
print("template_function_R04C14DEG_CD47KOvsWT_GSEAViz.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_R04C14_DEG_Volcano_CD47KOvsWT_copied<-readRDS(paste0(rds_output,"/var_R04C14_DEG_Volcano_CD47KOvsWT_copied.rds"))
Input_is_Seurat_count <- 0
#for(item in var_R04C14_DEG_Volcano_CD47KOvsWT_copied){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_R04C14_DEG_Volcano_CD47KOvsWT_copied<-as.data.frame(var_R04C14_DEG_Volcano_CD47KOvsWT_copied)}else{var_R04C14_DEG_Volcano_CD47KOvsWT_copied <- var_R04C14_DEG_Volcano_CD47KOvsWT_copied}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_R04C14DEG_CD47KOvsWT_GSEA<-readRDS(paste0(rds_output,"/var_R04C14DEG_CD47KOvsWT_GSEA.rds"))
Input_is_Seurat_count <- 0
#for(item in var_R04C14DEG_CD47KOvsWT_GSEA){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_R04C14DEG_CD47KOvsWT_GSEA<-as.data.frame(var_R04C14DEG_CD47KOvsWT_GSEA)}else{var_R04C14DEG_CD47KOvsWT_GSEA <- var_R04C14DEG_CD47KOvsWT_GSEA}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
...
var_R04C14DEG_CD47KOvsWT_GSEAViz<-R04C14DEG_CD47KOvsWT_GSEAViz(var_R04C14_DEG_Volcano_CD47KOvsWT_copied,var_R04C14DEG_CD47KOvsWT_GSEA,var_R04C14_Filtered_DEG,var_R04C14_Filtered_MetadataTable,var_GSEA_MSigDB_v6_2_with_Orthologs_CCBR_)
invisible(graphics.off())
saveRDS(var_R04C14DEG_CD47KOvsWT_GSEAViz, paste0(rds_output,"/var_R04C14DEG_CD47KOvsWT_GSEAViz.rds"))
