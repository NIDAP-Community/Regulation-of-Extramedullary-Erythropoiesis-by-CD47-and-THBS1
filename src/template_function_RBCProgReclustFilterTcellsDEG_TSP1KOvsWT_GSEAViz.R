...
RBCProgReclustFilterTcellsDEG_TSP1KOvsWT_GSEAViz <- function(RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment_Volcano_TSP1KOvsWT_copied, RBCProgReclustFilterTcellsDEG_TSP1KOvsWT_GSEAPreranked, RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment, RBCProg_ReclusteredCellTypes_FilterTCells_Metadata, GSEA_MSigDB_v6_2_with_Orthologs_CCBR_) {
...
## PARAMETERS
    ...
    plot_limit = "10"
    topline = 'coordinate'
    pathways = c("GSE26351_UNSTIM_VS_BMP_PATHWAY_STIM_HEMATOPOIETIC_PROGENITORS_DN","GSE26351_UNSTIM_VS_BMP_PATHWAY_STIM_HEMATOPOIETIC_PROGENITORS_UP","KEGG_HEDGEHOG_SIGNALING_PATHWAY","Signaling by Hedgehog")
    geneid = "Gene"
    geneid_gex = "Gene"
    contrasts = c()
    species = "Mouse"

    genescore_df <- RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment_Volcano_TSP1KOvsWT_copied
    gex_df <- RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment
    metadata_df <- RBCProg_ReclusteredCellTypes_FilterTCells_Metadata
    gsea_df <- RBCProgReclustFilterTcellsDEG_TSP1KOvsWT_GSEAPreranked
    pathway_db <- GSEA_MSigDB_v6_2_with_Orthologs_CCBR_
...
if (grepl("LE", which_plot)) {
    ...
    samples_to_include = setdiff(c(), geneid)
...
for ( i in 1:length(gsea_list) ) {
    ...
    if (which_plot == "ES+RNK+LE") {
        ...
        metadata = prep.metadata(meta=metadata_df, gsea=gsea, samples=colnames(gex_le), Sample="My_Sample_Names", Group="Treatment_Group", transformation=gex_transformation, dropREF=drop_ref, reference=reference_phenotype, own_palette=c(), label_palette='Accent', filterSamples=filter_samples)
...
png(filename="RBCProgReclustFilterTcellsDEG_TSP1KOvsWT_GSEAViz.png", width=(pageWidth+5)*300, height=(pageHeight+5)*300, units="px", pointsize=4, bg="white", res=300, type="cairo") 
print(preview)
...
print("template_function_RBCProgReclustFilterTcellsDEG_TSP1KOvsWT_GSEAViz.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment_Volcano_TSP1KOvsWT_copied<-readRDS(paste0(rds_output,"/var_RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment_Volcano_TSP1KOvsWT_copied.rds"))
Input_is_Seurat_count <- 0
#for(item in var_RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment_Volcano_TSP1KOvsWT_copied){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment_Volcano_TSP1KOvsWT_copied<-as.data.frame(var_RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment_Volcano_TSP1KOvsWT_copied)}else{var_RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment_Volcano_TSP1KOvsWT_copied <- var_RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment_Volcano_TSP1KOvsWT_copied}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_RBCProgReclustFilterTcellsDEG_TSP1KOvsWT_GSEAPreranked<-readRDS(paste0(rds_output,"/var_RBCProgReclustFilterTcellsDEG_TSP1KOvsWT_GSEAPreranked.rds"))
Input_is_Seurat_count <- 0
#for(item in var_RBCProgReclustFilterTcellsDEG_TSP1KOvsWT_GSEAPreranked){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_RBCProgReclustFilterTcellsDEG_TSP1KOvsWT_GSEAPreranked<-as.data.frame(var_RBCProgReclustFilterTcellsDEG_TSP1KOvsWT_GSEAPreranked)}else{var_RBCProgReclustFilterTcellsDEG_TSP1KOvsWT_GSEAPreranked <- var_RBCProgReclustFilterTcellsDEG_TSP1KOvsWT_GSEAPreranked}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment<-readRDS(paste0(rds_output,"/var_RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment.rds"))
Input_is_Seurat_count <- 0
#for(item in var_RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment<-as.data.frame(var_RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment)}else{var_RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment <- var_RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_RBCProg_ReclusteredCellTypes_FilterTCells_Metadata<-readRDS(paste0(rds_output,"/var_RBCProg_ReclusteredCellTypes_FilterTCells_Metadata.rds"))
Input_is_Seurat_count <- 0
#for(item in var_RBCProg_ReclusteredCellTypes_FilterTCells_Metadata){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_RBCProg_ReclusteredCellTypes_FilterTCells_Metadata<-as.data.frame(var_RBCProg_ReclusteredCellTypes_FilterTCells_Metadata)}else{var_RBCProg_ReclusteredCellTypes_FilterTCells_Metadata <- var_RBCProg_ReclusteredCellTypes_FilterTCells_Metadata}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_GSEA_MSigDB_v6_2_with_Orthologs_CCBR_<-readRDS(paste0(rds_output,"/var_GSEA_MSigDB_v6_2_with_Orthologs_CCBR_.rds"))
Input_is_Seurat_count <- 0
#for(item in var_GSEA_MSigDB_v6_2_with_Orthologs_CCBR_){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_GSEA_MSigDB_v6_2_with_Orthologs_CCBR_<-as.data.frame(var_GSEA_MSigDB_v6_2_with_Orthologs_CCBR_)}else{var_GSEA_MSigDB_v6_2_with_Orthologs_CCBR_ <- var_GSEA_MSigDB_v6_2_with_Orthologs_CCBR_}
invisible(graphics.off())
var_RBCProgReclustFilterTcellsDEG_TSP1KOvsWT_GSEAViz<-RBCProgReclustFilterTcellsDEG_TSP1KOvsWT_GSEAViz(var_RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment_Volcano_TSP1KOvsWT_copied,var_RBCProgReclustFilterTcellsDEG_TSP1KOvsWT_GSEAPreranked,var_RBCProg_ReclusteredCellTypes_FilterTCells_DEG_Treatment,var_RBCProg_ReclusteredCellTypes_FilterTCells_Metadata,var_GSEA_MSigDB_v6_2_with_Orthologs_CCBR_)
invisible(graphics.off())
saveRDS(var_RBCProgReclustFilterTcellsDEG_TSP1KOvsWT_GSEAViz, paste0(rds_output,"/var_RBCProgReclustFilterTcellsDEG_TSP1KOvsWT_GSEAViz.rds"))
