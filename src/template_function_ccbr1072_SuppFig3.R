# Filter Low Quality Cells [scRNA-seq][CCBR] (8f4529f6-de08-4aad-89fd-41df365a749f): v39
ccbr1072_SuppFig3 <- function(QC_ProcessedInput_SO) {
...
#    nidapLoadPackages(c("SCWorkflow","Seurat","reshape2","magrittr","dplyr","ggplot2","ggpubr","gridExtra","scDblFinder"))
    library("SCWorkflow")
    library("Seurat")
    library("reshape2")
    library("magrittr")
...
object = QC_ProcessedInput_SO

## Filter Samples
min.cells = 3
ncounts.limits=c("NA","2000")
mad.ncounts.limits=c("NA","NA")
nfeature.limits=c("NA","200")
mad.nfeature.limits=c("NA","NA")
mitoch.limits = c("15","NA")
mad.mitoch.limits = c("NA","NA")
complexity.limits = c("NA","NA")
mad.complexity.limits = c("NA","NA")
topNgenes.limits = c("NA","NA")
mad.topNgenes.limits = c("NA","NA")
n.topgnes=20
do.doublets.fitler=FALSE
...
# auto removed:   fs <- object$fileSystem()
#path <- "./rds_output/var_QC_ProcessedInput_SO.rds"
#  so <- readRDS(path)
so <- object
...
print("template_function_ccbr1072_SuppFig3.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_QC_ProcessedInput_SO<-readRDS(paste0(rds_output,"/var_QC_ProcessedInput_SO.rds"))
Input_is_Seurat_count <- 1
#for(item in var_QC_ProcessedInput_SO){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_QC_ProcessedInput_SO<-as.data.frame(var_QC_ProcessedInput_SO)}else{var_QC_ProcessedInput_SO <- var_QC_ProcessedInput_SO}
invisible(graphics.off())
var_ccbr1072_SuppFig3<-ccbr1072_SuppFig3(var_QC_ProcessedInput_SO)
invisible(graphics.off())
saveRDS(var_ccbr1072_SuppFig3, paste0(rds_output,"/var_ccbr1072_SuppFig3.rds"))
