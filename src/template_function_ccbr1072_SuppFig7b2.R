...
ccbr1072_SuppFig7b2 <- function(RBCProg_ReclusteredCellTypes_DEG_MouseRNAseq) {
    # image: png
    ...
    #Basic Parameters:
    df <- RBCProg_ReclusteredCellTypes_DEG_MouseRNAseq
    label.col <- "Gene"
    sig.col <- "p_val_Erythrocytes_vs_T_cells"
    pCutoff  = 0.05
    lfc.col <- "avg_log2FC_Erythrocytes_vs_T_cells"
    FCcutoff = 1.0
    ...
    title <- "Reclustered RBC Progenitors:  Erythroid Cells vs. T Cells"
...
#CHANGE LOG (NW 2022-03-22): Export significant counts to output log, remove from plot title (~line 160)
    ...
    png(
      filename="ccbr1072_SuppFig7b2.png",
      width=imageWidth,
      height=imageHeight,
      units="px",
      pointsize=4,
...
print("template_function_ccbr1072_SuppFig7b2.R #########################################################################")
...
var_ccbr1072_SuppFig7b2<-ccbr1072_SuppFig7b2(var_RBCProg_ReclusteredCellTypes_DEG_MouseRNAseq)
invisible(graphics.off())
saveRDS(var_ccbr1072_SuppFig7b2, paste0(rds_output,"/var_ccbr1072_SuppFig7b2.rds"))
